...
class Assessment(object):
    ...
    def get_annualized_return(self,df):
        ...
        """
        annualized_return = df.apply(lambda x: round((math.pow(x.tail(1).values[0] / x.head(1).values[0], (252 / df.shape[0])) - 1) * 100, 2))
    ...
    def percentage_industry(self):
        ...
        """
        rest_stock = list(set(self.select_dic.keys()).difference(set(self.hs300_list)).difference(set(self.csi500_list)))
    ...
    def bonus(self,x):
        ...
        """
        return 5 + 5 * self.sigmoid(x - self.score_dic['Comparison2'] * 0.01) + 5 * self.sigmoid(x - self.score_dic['Comparison2'] * 0.05) \
             + 5 * self.sigmoid(x - self.score_dic['Comparison2'] * 0.09) + 10 * self.sigmoid(x - self.score_dic['Comparison2'] * 0.15) \
             + 10 * self.sigmoid(x - self.score_dic['Comparison2'] * 0.30) + 10 * self.sigmoid(x - self.score_dic['Comparison2'] * 0.55) \
    ...
    def full_model(self, df_price_base, df_pctchg_base):
        ...
        if record_ar >= self.return_preference:
            ...
            print('评分：得%0.4f分。投资策略在投资周期的年化收益率%0.2f%%，可以满足用户的收益偏好（%0.2f%%）。' %(score_p11, record_ar,self.return_preference))
        else:
            print('评分：未得分。投资策略在投资周期的年化收益率为%0.2f%%，无法满足用户的收益偏好（%0.2f%%）。' %(record_ar,self.return_preference))

        if record_av <= self.risk_preference:
            score_p12 = self.score_dic['risk'] + math.log(self.risk_preference - record_av + 1)
            print('评分：得%0.4f分。投资策略在投资周期的年化波动率为%0.2f%%，可以满足用户的风险偏好（%0.2f%%）。' %(score_p12, record_av,self.risk_preference))
        else:
            print('评分：未得分。投资策略在投资周期的年化波动率为%0.2f%%，无法满足用户的风险偏好（%0.2f%%）。' %(record_av,self.risk_preference))

        if record_md <= self.risk_preference:
            score_p13 = self.score_dic['max_drawdown']
            print('评分：得%0.4f分。投资策略在投资周期的最大回撤率%0.2f%%，可以满足用户的风险偏好（%0.2f%%），用户投资体验感较好。' %(score_p13, record_md,self.risk_preference))
        else:
            print('评分：未得分。投资策略在投资周期的最大回撤率为%0.2f%%，无法满足用户的风险偏好（%0.2f%%），用户投资体验感较差。' %(record_md,self.risk_preference))   
        print()
        score_p1 = score_p11 + score_p12 + score_p13
        print('用户偏好：满分%0.4f，得分（含bonus）%0.4f。' %((self.score_dic['return'] + self.score_dic['risk'] + self.score_dic['max_drawdown']), score_p1))  
        ...
        base_price = base_scaled_return['hs300'] * perc['沪深300'] + base_scaled_return['csi500'] * perc['中证500'] + rest_price * perc['剩余股票']
        base_max = base_max_drawdown['hs300'] * perc['沪深300'] + base_max_drawdown['csi500'] * perc['中证500'] + rest_max * perc['剩余股票']
        ...
        if corr['投资组合']['业绩基准'] >= 0.70:
            if select_sharpe > base_s_r['hs300']:
                ...
                print('评分：得%0.4f分。投资策略在投资周期的夏普比率为%0.2f%%，可以跑赢沪深300指数（%0.2f%%）。' %(score_sub11, select_sharpe, base_s_r['hs300']))
            else:
                print('评分：未得分。投资策略在投资周期的夏普比率为%0.2f%%，无法跑赢沪深300指数（%0.2f%%）。' %(select_sharpe, base_s_r['hs300']))
            if select_sharpe > base_s_r['csi500']:
                score_sub12 = self.score_dic['Comparison1'][1] + math.log(select_sharpe - base_s_r['csi500'] + 1)
                print('评分：得%0.4f分。投资策略在投资周期的夏普比率为%0.2f%%，可以跑赢中证500指数（%0.2f%%）。' %(score_sub12, select_sharpe, base_s_r['csi500']))
            else:
                print('评分：未得分。投资策略在投资周期的夏普比率为%0.2f%%，无法跑赢中证500指数（%0.2f%%）。' %(select_sharpe, base_s_r['csi500']))
            ...
            if select_sharpe > base_sharpe:
                ...
                print('评分：得%0.4f分。投资策略在投资周期的夏普比率为%0.2f%%，可以跑赢业绩基准（%0.2f%%）。' %(score_sub13, select_sharpe, base_sharpe))
            else:
                print('评分：未得分。投资策略在投资周期的夏普比率为%0.2f%%，无法跑赢业绩基准（%0.2f%%）。' %(select_sharpe, base_sharpe))
        ...
        i_r_csi500 = self.get_information_ratio(record_ar, a_r_base['csi500'], record_ar_daily, a_r_daily_base['csi500'])
        ...
        for i in self.select_dic.keys():
            if i in self.Astock['ts_code'].values and i in a_r:
                ...
                for day in pre_test:
                    ...
                    if int(rank_ar/len(s1) * 100) <= (100-self.return_preference) and int(rank_av/len(s2) * 100) <= self.risk_preference:
                        days.append(day)
        ...
        print()
        print('同类股票对比：满分%0.4f，得分（含bonus）%0.4f。' %(self.score_dic['Comparison2'], score_sub2 + score_sub2_bonus))
        score = score_p1 + score_sub1 + score_sub2 + score_sub2_bonus
        print('-' * 90)
        print('-' * 90)
        print('收益风险评估总得分(含bonus)%0.4f。打分明细：%0.4f = %0.4f + %0.4f + %0.4f + %0.4f。' %(score, score, score_p1, score_sub1, score_sub2, score_sub2_bonus))
        print()
...
class Get_Score(object):
    ...
    def full_model(self): 
        ...
        # 读取
        js = json.load(open(self.Astock_file))                                                           
        ...
        df, score = A.full_model(df_price_base, df_pctchg_base)
